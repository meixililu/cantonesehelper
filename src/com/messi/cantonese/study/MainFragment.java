...
public class MainFragment extends Fragment implements OnClickListener {
	...
	private void init() {
		...
		translate_to_ch_btn = (FrameLayout) view.findViewById(R.id.translate_to_ch_btn);
		translate_to_yy_btn = (FrameLayout) view.findViewById(R.id.translate_to_yy_btn);
		...
		translate_to_yy_btn.setOnClickListener(this);
		translate_to_ch_btn.setOnClickListener(this);
		...
		if(selectedLanguage.equals(XFUtil.AccentCH)){
	...
	private String getSpeakLanguage(){
		return mSharedPreferences.getString(this.getString(R.string.preference_key_accent), XFUtil.AccentCH);
	}
	...
	private void setSpeakLanguage(String language){
		...
		if(language.equals(XFUtil.AccentCH)){
			mEditor.putString(this.getString(R.string.preference_key_recognizer),XFUtil.VoiceEngineCH);
			mEditor.putString(this.getString(R.string.preference_key_accent),XFUtil.AccentCH);
		}else if(language.equals(XFUtil.AccentHK)){
			mEditor.putString(this.getString(R.string.preference_key_recognizer), XFUtil.VoiceEngineCH);
			mEditor.putString(this.getString(R.string.preference_key_accent), XFUtil.AccentHK);
		}
		mEditor.commit();
	}
	...
	private void RequestAsyncTask(){
		WXEntryActivity.mWXEntryActivity.setSupportProgressBarIndeterminateVisibility(true);
		WXEntryActivity.mWXEntryActivity.setSupportProgressBarVisibility(true);
		translate_to_ch_btn.setEnabled(false);
		...
		LanguagehelperHttpClient.post(mRequestParams, new TextHttpResponseHandler() {
			...
			public void onFinish() {
				...
				WXEntryActivity.mWXEntryActivity.setSupportProgressBarIndeterminateVisibility(false);
				WXEntryActivity.mWXEntryActivity.setSupportProgressBarVisibility(false);
				translate_to_ch_btn.setEnabled(true);
			}
			@Override
			...
			public void onSuccess(int statusCode, Header[] headers, String responseString) {
				if (!TextUtils.isEmpty(responseString)) {
					...
					if (dstString.contains("error_msg:")) {
						...
						currentDialogBean.setQuestion_lan(Settings.from);
						currentDialogBean.setResult_lan(Settings.to);
	...
	private void hideIME(){
		...
		imm.hideSoftInputFromWindow(translate_to_ch_btn.getWindowToken(), 0); 
	}
	 ...
	 * submit request task
	 */
	private void submit(boolean isToCantonese){
		if(isToCantonese){
			Settings.from = "zh";
			Settings.to = "yue";
		}else{
